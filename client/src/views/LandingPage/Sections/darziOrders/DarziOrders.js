...
export default function DarziOrders(props) {
  ...
  const handleOrder = async (Itemid, DarziId, status, DarziName) => {
    console.log(Itemid, DarziId, status, DarziName);
    var myModule = require("views/database");
    const user = await axios.post(
      myModule.servername + "/api/users/UpdateDarzi/",
      {
        DarziId: DarziId,
        OrderId: Itemid,
        status: status,
      ...
      >
        ...
        >
          ...
          <Tbody>
            {data.map((item, idx) => {
              return (
                item.DarziId === localStorage.getItem("darziwebtoken") && (
                  <Tr key={item._id}>
                    ...
                    <Td>
                      ...
                      {item.OrderStatus === "Completed" &&
                        "Order Completed Waiting For Pickup"}
                      {(item.OrderStatus === "Request Delivery" ||
                        item.OrderStatus === "Accept Delivery") &&
                        "Order Completed"}
                    </Td>
                    <Td>
                      {item.OrderStatus === "Completed" && (
                        ...
                        >
                          <CheckCircleIcon /> Deliver Order
                        </Button>
                      )}
                      {item.OrderStatus === "Request Delivery" &&
                        "Waiting for accepting delivery"}
                      {item.OrderStatus === "Accept Delivery" &&
                        "Order Delivered"}
                    </Td>
                  </Tr>
